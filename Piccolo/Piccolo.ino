...
byte
  levelDisplay[8],  // The current level to display the bar at
...
void DrawScreen()
{
  ...
  for (uint8_t i = 0; i < 8; ++i)
  {
    ...
    //if (peak[i] == 0) matrix.drawLine(i, 0, i, 7, LED_OFF);
    //else if (levelDisplay[i] < 8) matrix.drawLine(i, 0, i, 7 - levelDisplay[i], LED_OFF);

    
    uint8_t peakDisplay = 8 - peak[i];
    if (peakDisplay < 2)      matrix.drawPixel(i, peakDisplay, LED_RED);
    else if (peakDisplay < 6) matrix.drawPixel(i, peakDisplay, LED_YELLOW);
    else                      matrix.drawPixel(i, peakDisplay, LED_GREEN);
  }
